...
function Registration() {
    ...
    const SIGNUP = [
        ...
        },
        {
            ...
            value:user.opening,
            type:'time',
            text:'Enter Your Hospital Opening Time'
        },
        {
            name:'ending',
            value:user.ending,
            type:'time',
            text:'Enter Your Hospital Closing Time'
        },
        {
            name:'password',
            value:user.password,
    ...
    const registerHospital = async(e)=>{
        ...
        if(!user.email_id || !user.password || !user.name){
            setError("You have not entered valid Email Id , Password and Hospital Name. Please enter mentioned fields properly")
            setShow(true)
        }else{
            try{
                ...
                try {
                    ...
                    setError("Your Hospital Registration is Successful")
                    setUser('')
                    setShow(true)
                  } catch (e) {
                    setAdminLoading(false)
                ...
                setAdminLoading(false)
                setError("Your Hospital is already Registered ! Please try to Login OR Contact to the Site Admin.")
                setShow(true)
            }
        }
    }
            ...
            <Form className="account_form_body mt-2">
                ...
                <Row>
                    <Col xl={6} className="account_button text-center mt-2">
                        ...
                        <p>You are already Registered, Please <Link to="/loginhospital">Login Here.</Link></p>
                    </Col>
                </Row>
            </Form>
        </Container>
